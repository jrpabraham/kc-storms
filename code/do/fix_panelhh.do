...
set obs 6403
replace year = 1990 in 6403
set obs 6404
replace year = 2012 in 6404
...
/* Many-to-one merge of storm and KALAHI by province/year */
...
//egen mun_year = group(mun year)

foreach x of varlist maxs {
	foreach y of varlist lntoexp lnfood lnnonfood lnpcexp lnpcfood lnpcnonfoo lntrexp agr pov_self1 water1 electr1 healthcr1 school1 bayanihan give_time give_cash assembly attend_m attend_f {
	
	qui xtreg `y' i.treat L(0/4).`x', fe vce(cluster mun)
	outreg2 using `y'_`x'reg.doc , addtext(FE, Household, Time Trend, None)
	
	qui xtreg `y' i.treat L(0/4).`x' i.year, fe vce(cluster mun)
	outreg2 using `y'_`x'reg.doc , append addtext(FE, Household, Time Trend, Year FE)
	
	qui xtreg `y' i.treat L(0/4).`x' year, fe vce(cluster mun)
	outreg2 using `y'_`x'reg.doc , append addtext(FE, Household, Time Trend, Linear)

	qui xtreg `y' i.treat##c.L(0/4).`x', fe vce(cluster mun)
	outreg2 using `y'_`x'reg.doc , append addtext(FE, Household, Time Trend, None)
